...
const yargs_1 = __importDefault(__nccwpck_require__(822));
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            try {
                xcodeBuildOutput = child_process.execSync('xcodebuild -version', {
                    encoding: 'utf8'
                });
            }
            catch (error) {
            ...
            try {
                if (isXcode16OrHigher) {
                    ...
                    if (debug)
                        console.log(`Running: ${testResultsCommand}`);
                    const testResultsOutput = child_process.execSync(testResultsCommand, {
                        encoding: 'utf8'
                    });
                    ...
                    if (testResults.testFailures &&
                        Object.keys(testResults.testFailures).length > 0) {
                    ...
                    // Add device info
                    if (testResults.devicesAndConfigurations &&
                        testResults.devicesAndConfigurations.device) {
                    ...
                    if (debug)
                        console.log(`Running: ${summaryCommand}`);
                                    ...
                                    if (test.subtests) {
                                        for (const subtest of test.subtests) {
                                            if (subtest.subtests) {
                                                ...
                                                const testName = ((_a = subtest.name) === null || _a === void 0 ? void 0 : _a._value) || 'Unknown Test';
                                                const testStatus = ((_b = subtest.testStatus) === null || _b === void 0 ? void 0 : _b._value) || 'unknown';
                                                const duration = ((_d = (_c = subtest.duration) === null || _c === void 0 ? void 0 : _c._value) === null || _d === void 0 ? void 0 : _d.toFixed(2)) || '?';
                ...
                testSummary += `Failed to process test results: ${error.message}\n`;
            }
            ...
            if (argv['show-code-coverage']) {
                try {
                    if (isXcode16OrHigher) {
                        ...
                        if (debug)
                            console.log(`Running: ${targetsCommand}`);
                        const targetsOutput = child_process.execSync(targetsCommand, {
                            encoding: 'utf8'
                        });
                        ...
                        if (targets.length > 0) {
                            ...
                            try {
                                ...
                                child_process.execSync('xcrun xccov view --help', {
                                    encoding: 'utf8'
                                });
                            }
                            catch (helpError) {
                                ...
                                if (debug)
                                    console.log('--only-target flag not supported, using alternative approach');
                            }
                            for (const targetLine of targets) {
                                ...
                                if (onlyTargetSupported) {
                                    try {
                                        ...
                                        if (debug)
                                            console.log(`Running: ${targetCommand}`);
                                        const targetOutput = child_process.execSync(targetCommand, {
                                            encoding: 'utf8'
                                        });
                                        coverageSummary += '```\n' + targetOutput + '```\n\n';
                                    }
                                        ...
                                        coverageSummary += `Error getting details for target: ${targetError.message}\n\n`;
                                    }
                                }
                                else {
                        ...
                        const summary = JSON.parse(summaryOutput);
                        if (summary.actions &&
                            summary.actions.codeCoverageInfo &&
                            summary.actions.codeCoverageInfo.archiveRef) {
                            const archiveRef = summary.actions.codeCoverageInfo.archiveRef._value;
                            if (debug)
                                console.log(`Found coverage archive reference: ${archiveRef}`);
                            ...
                            if (debug)
                                console.log(`Running: ${xccovCommand}`);
                            const coverageOutput = child_process.execSync(xccovCommand, {
                                encoding: 'utf8'
                            });
                            coverageSummary = '# Code Coverage Summary\n\n';
                            coverageSummary += '```\n' + coverageOutput + '```\n\n';
                            ...
                            coverageSummary = '# Code Coverage Summary\n\n';
                            coverageSummary +=
                                'No code coverage data found in the xcresult bundle.\n';
                        }
                    }
                }
                catch (error) {
                    console.error(`Error processing code coverage: ${error.message}`);
                    coverageSummary = '# Error Processing Code Coverage\n\n';
                    coverageSummary += `Failed to process code coverage: ${error.message}\n`;
                }
            }
            // Output the report
...
const {Yargs, processArgv} = __nccwpck_require__(562);
...
/******/ 	var __webpack_exports__ = __nccwpck_require__(__nccwpck_require__.s = 733);

...
async function main() {
  try {
    ...
    const isXcode16OrHigher =
      xcodeVersionMatch && parseInt(xcodeVersionMatch[1]) >= 16

    if (debug) {
      console.log(`Detected Xcode version: ${xcodeBuildOutput.trim()}`)
      console.log(
        `Using ${isXcode16OrHigher ? 'new' : 'legacy'} command format`
      )
    }
    ...
    try {
      if (isXcode16OrHigher) {
        ...
        // Add device info
        if (
          testResults.devicesAndConfigurations &&
        ...
        const infoPlist = child_process.execSync(
          `plutil -p "${bundlePath}/Info.plist"`,
          {encoding: 'utf8'}
        )
        const rootIdMatch = infoPlist.match(
          /"rootId"\s*=>\s*{\s*"hash"\s*=>\s*"([^"]+)"/
        )
        ...
        if (summary.actions && summary.actions.testsRef) {
          ...
          if (testData.summaries && testData.summaries.length > 0) {
            ...
            testSummary += `- Test duration: ${testSummaryData.duration._value.toFixed(
              2
            )} seconds\n`
          }
                ...
                if (test.subtests) {
                  for (const subtest of test.subtests) {
                    if (subtest.subtests) {
                      ...
                      const duration =
                        subtest.duration?._value?.toFixed(2) || '?'
      ...
      console.error(
        `Error processing test results: ${(error as Error).message}`
      )
      testSummary = '# Error Processing Test Results\n\n'
      testSummary += `Failed to process test results: ${
    ...
    if (argv['show-code-coverage']) {
      try {
        if (isXcode16OrHigher) {
          ...
          if (targets.length > 0) {
            ...
            try {
              ...
              if (debug)
                console.log(
                  '--only-target flag not supported, using alternative approach'
                )
            }
            ...
            for (const targetLine of targets) {
              ...
              const targetMatch = targetLine.match(
                /^[0-9A-F]+ (.+?)\s+\d+\s+\d+\.\d+%$/
              )
              ...
              if (onlyTargetSupported) {
                try {
                  ...
                  coverageSummary += `Error getting details for target: ${
        ...
        console.error(
          `Error processing code coverage: ${(error as Error).message}`
        )
        coverageSummary = '# Error Processing Code Coverage\n\n'
        coverageSummary += `Failed to process code coverage: ${
          (error as Error).message
        }\n`
      }
    }
